...
class TagMatcher:
    """Semantic matching service for cross-exchange tags"""

    SIMILARITY_THRESHOLD = 0.4  # Lower threshold for tags since they are shorter

    # Common tag synonyms and related terms
    TAG_SYNONYMS = {
        'politics': {'political', 'government', 'election', 'elections', 'congress', 'senate'},
        'crypto': {'cryptocurrency', 'bitcoin', 'ethereum', 'blockchain', 'digital currency'},
        'economics': {'economy', 'economic', 'finance', 'financial', 'markets'},
        'sports': {'sport', 'athletics', 'games'},
        'entertainment': {'media', 'celebrities', 'celebrity', 'movies', 'music'},
        'tech': {'technology', 'ai', 'artificial intelligence', 'software', 'computing'},
    ...
    def expand_synonyms(text: str) -> str:
        """Expand tag with synonyms for better matching"""
        text = text.lower()
        expanded = text
        for base, synonyms in TagMatcher.TAG_SYNONYMS.items():
            if base in text or any(syn in text for syn in synonyms):
                expanded += ' ' + base + ' ' + ' '.join(synonyms)
        return expanded

    def compute_tag_similarity(self, tag1: str, tag2: str) -> Tuple[float, Dict]:
        """Compute similarity between two tag labels"""
        ...
        if norm1 in norm2 or norm2 in norm1:
            ...
            breakdown['containment'] = True
        breakdown['containment_score'] = containment_score
        ...
        if words1 and words2:
            ...
            if common_words:
                word_overlap_score = len(common_words) / len(words1 | words2)
                breakdown['common_words'] = list(common_words)
        breakdown['word_overlap_score'] = word_overlap_score

        # Synonym expansion and matching
        expanded1 = self.expand_synonyms(tag1)
        expanded2 = self.expand_synonyms(tag2)
        exp_words1 = set(expanded1.split())
        exp_words2 = set(expanded2.split())
        synonym_score = 0.0
        if exp_words1 and exp_words2:
            common_expanded = exp_words1 & exp_words2
            if common_expanded:
                synonym_score = len(common_expanded) / max(len(exp_words1), len(exp_words2))
        breakdown['synonym_score'] = synonym_score

        # TF-IDF similarity on expanded text
        tfidf_score = 0.0
        try:
            tfidf_matrix = self.vectorizer.fit_transform([expanded1, expanded2])
            tfidf_score = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:2])[0][0]
        except Exception:
            pass
        breakdown['tfidf_score'] = tfidf_score
        ...
        # Combined score with weights
        combined_score = (
            0.35 * containment_score +   # One tag contains the other
            0.25 * word_overlap_score +  # Direct word overlap
            0.20 * synonym_score +       # Synonym-based overlap
            0.20 * tfidf_score           # TF-IDF similarity
        )

        breakdown['combined_score'] = combined_score
        return combined_score, breakdown

    def generate_tag_match_reason(self, breakdown: Dict) -> str:
        """Generate human-readable match reason"""
        reasons = []

        if breakdown.get('exact_match'):
            reasons.append("Exact match")
        elif breakdown.get('containment'):
            reasons.append("Substring match")
        if breakdown.get('common_words'):
            reasons.append(f"Common words: {', '.join(breakdown['common_words'][:3])}")
    ...
    def find_tag_matches(
        self,
        kalshi_tags: List[Tag],
        polymarket_tags: List[Tag],
        threshold: float = None
    ) -> List[Tuple[Tag, Tag, float, str]]:
        """
        Find matching tags between Kalshi and Polymarket.
        Returns list of tuples: (kalshi_tag, poly_tag, similarity_score, reason)
        """
        threshold = threshold or self.SIMILARITY_THRESHOLD
        matches = []

        if not kalshi_tags or not polymarket_tags:
            return matches

        # For each Kalshi tag, find the best matching Polymarket tag
        for kalshi_tag in kalshi_tags:
            best_match = None
            best_score = 0
            best_breakdown = {}

            # Include category in the matching for Kalshi tags
            kalshi_text = kalshi_tag.label
            if kalshi_tag.category:
                kalshi_text = f"{kalshi_tag.category} {kalshi_tag.label}"

            for poly_tag in polymarket_tags:
                score, breakdown = self.compute_tag_similarity(
                    kalshi_text,
                    poly_tag.label
                )
            ...
            if best_match and best_score >= threshold:
                reason = self.generate_tag_match_reason(best_breakdown)
                matches.append((kalshi_tag, best_match, best_score, reason))

        # Also check reverse direction - Polymarket tags that might match Kalshi tags
        # This ensures we don't miss any potential matches
        matched_poly_ids = {m[1].id for m in matches}
        for poly_tag in polymarket_tags:
            if poly_tag.id in matched_poly_ids:
                continue
            ...
            for kalshi_tag in kalshi_tags:
                # Skip if this Kalshi tag already has a match with this poly tag
                if any(m[0].id == kalshi_tag.id and m[1].id == poly_tag.id for m in matches):
                    continue
                ...
                if score > best_score:
                    best_score = score
                    best_match = kalshi_tag
                    best_breakdown = breakdown

            if best_match and best_score >= threshold:
                # Check if this pair doesn't already exist
                if not any(m[0].id == best_match.id and m[1].id == poly_tag.id for m in matches):
                    reason = self.generate_tag_match_reason(best_breakdown)
                    matches.append((best_match, poly_tag, best_score, reason))
    ...
    def create_tag_match_records(
        self,
        matches: List[Tuple[Tag, Tag, float, str]],
        is_manual: bool = False
    ) -> List[TagMatch]:
        """Create or update TagMatch records from matches"""
        created_matches = []

        for kalshi_tag, poly_tag, score, reason in matches:
            match, created = TagMatch.objects.update_or_create(
                kalshi_tag=kalshi_tag,
                polymarket_tag=poly_tag,
                defaults={
                    'similarity_score': score,
                    'match_reason': reason,
                    'is_manual': is_manual
    ...
    def auto_match_all_tags(self) -> List[TagMatch]:
        """Automatically find and save all tag matches"""
        kalshi_tags = list(Tag.objects.filter(exchange=Exchange.KALSHI))
        poly_tags = list(Tag.objects.filter(exchange=Exchange.POLYMARKET))

        matches = self.find_tag_matches(kalshi_tags, poly_tags)
        return self.create_tag_match_records(matches, is_manual=False)
